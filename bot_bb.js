...
async function run() {
  ...
  if (!CONFIG.paperTrading) {
    const paperOpen = positions.open.filter(p => p.orderId?.startsWith("BB-PAPER-"));
    if (paperOpen.length > 0) {
      console.log(`⚠️ [C] 偵測到 PAPER→LIVE 切換，清除 ${paperOpen.length} 筆模擬持倉，本輪跳過進場`);
      positions.open = positions.open.filter(p => !p.orderId?.startsWith("BB-PAPER-"));
      savePositions(positions);
      return;
    }
  }
