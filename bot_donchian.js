 ...
 * 牛市（BTC > MA200日線）：4H收盤突破 N120根最高 → 做多，SL = N60根最低（上移）
 * 熊市（BTC < MA200日線）：4H收盤突破 N120根最低 → 做空，SL = N60根最高（下移）
 * N120=20天/N60=10天（4H等效日線）| 止盈 3:1 | 每15分鐘掃一次
 * 回測（18個月45幣雙向,含手續費）：20天突破 PF 1.48 / +$641，樣本內外皆正(1.44/1.56)；
 *   對比舊 30天 PF 1.27 且樣本內為負(0.91)——2026-09-24 由 30天→20天。
 */
...
// ─── 策略參數 ─────────────────────────────────────────────────────────────────
const N_HIGH      = 120;   // 突破：前120根4H（= 20天）
const N_LOW       = 60;    // 止損：前60根4H（= 10天）
